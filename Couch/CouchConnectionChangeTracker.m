...
willSendRequestForAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge
{
    ...
    if ($equal(authMethod, NSURLAuthenticationMethodServerTrust)) {
        ...
        if ([CouchRemoteRequest checkTrust: trust forHost: host]) {
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
    CouchStatus status = (CouchStatus) ((NSHTTPURLResponse*)response).statusCode;
    ...
    if (CouchStatusIsError(status)) {
        Warn(@"%@: Got status %i for %@", self, status, _databaseURL);
        [self connection: connection
        didFailWithError: CouchStatusToNSError(status, self.changesFeedURL)];
    } else {
