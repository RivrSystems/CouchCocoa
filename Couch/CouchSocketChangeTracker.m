...
- (BOOL) checkSSLCert {
    ...
    if (sslTrust) {
        ...
        BOOL trusted = [CouchRemoteRequest checkTrust: sslTrust forHost: url.host];
        CFRelease(sslTrust);
        if (!trusted) {
...
- (BOOL) readResponseHeader {
    ...
    if (status == 401 || status == 407) {
        ...
        if (!_credential && !authorization) {
            ...
            if (_credential) {
                ...
                [self errorOccurred: CouchStatusToNSError((CouchStatus)status, self.changesFeedURL)];
                return NO;
            }
        }
    ...
    CFRelease(response);
    if (status >= 300) {
        self.error = CouchStatusToNSErrorWithInfo(status, self.changesFeedURL, errorInfo);
        [self stop];
        return NO;
    }
    _retryCount = 0;
...
- (void) asyncParseChangeLines: (NSArray*)lines {
    ...
    [sParseQueue addOperationWithBlock: ^{
        ...
        for (NSData* line in lines) {
            id change = [CouchJSON JSONObjectWithData: line options: 0 error: NULL];
            if (!change) {
                Warn(@"CouchSocketChangeTracker received unparseable change line from server: %@", [line my_UTF8ToString]);
                break;
            }
