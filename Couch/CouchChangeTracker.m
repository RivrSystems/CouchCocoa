...
- (id)initWithDatabaseURL: (NSURL*)databaseURL
    ...
    if (self) {
        if([self class] == [CouchChangeTracker class]) {
            // CouchChangeTracker is abstract; instantiate a concrete subclass instead.
            return [[CouchSocketChangeTracker alloc] initWithDatabaseURL: databaseURL
                                                                 mode: mode
...
- (NSString*) changesFeedPath {
    ...
    if (seq) {
        ...
        if ([seq isKindOfClass: [NSArray class]] || [seq isKindOfClass: [NSDictionary class]])
            seq = [CouchJSON stringWithJSONObject: seq options: 0 error: nil];
        [path appendFormat: @"&since=%@", CouchEscapeURLParam([seq description])];
    }
    if (_limit > 0)
        [path appendFormat: @"&limit=%u", _limit];
    if (_filterName) {
        [path appendFormat: @"&filter=%@", CouchEscapeURLParam(_filterName)];
        for (NSString* key in _filterParameters) {
            ...
            if (![value isKindOfClass: [NSString class]]) {
                ...
                NSError* error;
                value = [CouchJSON stringWithJSONObject: value options: CouchJSONWritingAllowFragments
                                               error: &error];
                if (!value) {
            ...
            [path appendFormat: @"&%@=%@", CouchEscapeURLParam(key),
             CouchEscapeURLParam(value)];
        }
    }
    ...
    if (_docIDs) {
        ...
        if (_filterName) {
            ...
            NSError *error;
            NSString *docIDsParam = [CouchJSON stringWithJSONObject: _docIDs options: CouchJSONWritingAllowFragments
                                                           error: &error];
            if (!docIDsParam || error) {
                Warn(@"Illegal doc IDs %@, %@", [_docIDs description], [error localizedDescription]);
            }
            [path appendFormat:@"&filter=_doc_ids&doc_ids=%@", CouchEscapeURLParam(docIDsParam)];
        }
    }
...
- (NSURL*) changesFeedURL {
    return CouchAppendToURL(_databaseURL, self.changesFeedPath);
}
...
- (void) setUpstreamError: (NSString*)message {
    ...
    self.error = [NSError errorWithDomain: @"CouchChangeTracker" code: kCouchStatusUpstreamError userInfo: nil];
}
...
- (void) stopped {
    ...
    id<CouchChangeTrackerClient> client = _client;
    _client = nil;
            ...
            if (![self receivedChange: change]) {
                if (errorMessage) {
                    *errorMessage = $sprintf(@"Invalid change object: %@",
                                             [CouchJSON stringWithJSONObject: change
                                                                  options:CouchJSONWritingAllowFragments
                                                                    error: nil]);
                }
                return NO;
            }
...
- (NSInteger) receivedPollResponse: (NSData*)body errorMessage: (NSString**)errorMessage {
    ...
    NSError* error;
    id changeObj = [CouchJSON JSONObjectWithData: body options: 0 error: &error];
    if (!changeObj) {
