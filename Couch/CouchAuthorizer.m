...
@implementation CouchBasicAuthorizer
...
{
    Log(@"CouchBasicAuthorizer initWith <%@>", url);//TEMP
                         ...
                         forRealm: (NSString*)realm
{
    ...
    if (username && password) {
        ...
        seekrit = [RESTBase64 encode: [seekrit dataUsingEncoding: NSUTF8StringEncoding]];
...
#if 0
// If enabled, these methods would make CouchCocoa use cookie-based login intstead of basic auth;
...
@implementation CouchMACAuthorizer
         ...
         issueTime: (NSDate*)issueTime
{
    ...
    if (self) {
        ...
        if ([algorithm isEqualToString: @"hmac-sha-1"])
            _hmacFunction = &CouchHMACSHA1;
        else if ([algorithm isEqualToString: @"hmac-sha-256"])
            _hmacFunction = &CouchHMACSHA256;
        else {
            return nil;
        }
    }
                         ...
                         body: (NSData*)body
{
    ...
    return nil;
    NSString* nonce = $sprintf(@"%.0f:%@", -[_issueTime timeIntervalSinceNow], CouchCreateUUID());
    NSString* ext = @"";  // not implemented yet
    
    NSString* bodyHash = @"";
    if (body.length > 0) {
        NSData* digest = (_hmacFunction == &CouchHMACSHA1) ? CouchSHA1Digest(body) : CouchSHA256Digest(body);
        bodyHash = [RESTBase64 encode: digest];
    }
    ...
    NSString* normalized = $sprintf(@"%@\n%@%@\n%@\n%d\n%@\n%@\n",
                                    ...
                                    ext);
    NSString* mac;
    mac = [RESTBase64 encode: _hmacFunction([_key dataUsingEncoding: NSUTF8StringEncoding],
