...
@implementation CouchRemoteRequest


+ (NSString*) userAgentHeader {
    return $sprintf(@"CouchCocoa/%@", @"1.2");
}
...
- (id<CouchAuthorizer>) authorizer {
    return _authorizer;
}

- (void) setAuthorizer: (id<CouchAuthorizer>)authorizer {
...
- (void) cancelWithStatus: (int)status {
    [_connection cancel];
    [self connection: _connection didFailWithError: CouchStatusToNSError(status, _request.URL)];
}
...
- (bool) retryWithCredential {
    ...
    [_connection cancel];
    self.authorizer = [[CouchBasicAuthorizer alloc] initWithCredential: cred];
    ...
    if ($equal(authMethod, NSURLAuthenticationMethodHTTPBasic) ||
        ...
        if (challenge.previousFailureCount <= 1) {
            ...
            if (cred) {
                ...
                _authorizer = [[CouchBasicAuthorizer alloc] initWithCredential: cred];
                return;
            }
        }
...
#if DEBUG
    if (!CouchStatusIsError(_status)) {
        // By setting the user default "CouchFakeFailureRate" to a number between 0.0 and 1.0,
        // you can artificially cause failures of that fraction of requests, for testing.
        // The status will be 567, or the value of "CouchFakeFailureStatus" if it's set.
        NSUserDefaults* dflts = [NSUserDefaults standardUserDefaults];
        float fakeFailureRate = [dflts floatForKey: @"CouchFakeFailureRate"];
        if (fakeFailureRate > 0.0 && random() < fakeFailureRate * 0x7FFFFFFF) {
            AlwaysLog(@"***FAKE FAILURE: %@", self);
            _status = (int)[dflts integerForKey: @"CouchFakeFailureStatus"] ?: 567;
        }
    }
#endif
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    if (WillLog()) {
        if (!(_dontLog404 && error.code == kCouchStatusNotFound && $equal(error.domain, CouchHTTPErrorDomain)))
            Log(@"%@: Got error %@", self, error);
    }
    
    // If the error is likely transient, retry:
    if (CouchMayBeTransientError(error) && [self retry])
        return;
...
@implementation CouchRemoteJSONRequest

- (void) setupRequest: (NSMutableURLRequest*)request withBody: (id)body {
    [request setValue: @"application/json" forHTTPHeaderField: @"Accept"];
    if (body) {
        request.HTTPBody = [CouchJSON dataWithJSONObject: body options: 0 error: NULL];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if (_jsonBuffer.length > 0) {
        result = [CouchJSON JSONObjectWithData: _jsonBuffer options: 0 error: NULL];
        if (!result) {
            Warn(@"%@: %@ %@ returned unparseable data '%@'",
                 self, _request.HTTPMethod, _request.URL, [_jsonBuffer my_UTF8ToString]);
            error = CouchStatusToNSError(kCouchStatusUpstreamError, _request.URL);
        }
    } else {
        result = $dict();
    }
